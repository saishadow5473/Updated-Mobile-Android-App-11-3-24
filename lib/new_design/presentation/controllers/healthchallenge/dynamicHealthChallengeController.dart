...
class DynamicHealthChallengeController extends GetxController {
  ...
  Future getEnrolledCompletedList()async{
    SharedPreferences prefs1 = await SharedPreferences.getInstance();
    String userid = prefs1.getString("ihlUserId");
     sortedEnrolledCompleted = await ChallengeApi().getSortedEnrolledList(userid: userid);
  }
  Future fetchNewChallengeList() async {
    int _end = _start + 20;
    _isFetching = true;
    ...
    } else if (listofChallenges.isNotEmpty && !_isLoading) {
      update([_ListChallengeUpdateId]);
    }
    // if there are more challenges, fetch the next page
    if (!_lastPage) {
      fetchNewChallengeList();
    }
  }
